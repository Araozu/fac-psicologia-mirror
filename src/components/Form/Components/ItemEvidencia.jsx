...
export default function ItemEvidencia(props){
    let {id, denominacion, indexOnList, onDelete} = props;
    ...
    return (
        <>
            <div className='form-input-buttons-group secondary'>
                <input key={id} id={id} disabled={true} value={denominacion+'.pdf'} type="text" className={"form-input-text"}/>
                <a onClick={handleDownload} className="ml-2 mr-2 download-a" download>DESCARGAR</a>
                <a className="form-icon-button form-delete-button" onClick={(e) => {onDelete(id, indexOnList)}}> <i class="fa-solid fa-trash"></i></a>
            </div>
        </>
    );
}
