...
export default function InputTextDinamics(props){
     ...
     */
    let {name, label, description, initialValues, onChange} = props;
        ...
        <>
            ...
            <div>
                <div className="form-input-buttons-group">
                    ...
                    <a className="form-icon-button form-add-button" onClick={handleAdd}> <i class="fa-solid fa-plus"></i> </a>
                </div>
                <hr />
                <div className="input-list-container">
                    {values?.map( (item, i) => <ItemList key={item.name+'-'+i} onChange={handleEdit} canEdit={true} idItem={item.id} value={item.value} id={name+'-'+item.id} indexOnList={i} onDelete={handleDelete}/> )}
                </div>   
            </div>
        </>
    );
